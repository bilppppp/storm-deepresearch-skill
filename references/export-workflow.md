...
After validation passes, use `storm_research.py collect "$RUN_DIR" --to <new-dir>` for local handoff. Captured host execution collect copies report Markdown, HTML, PDF, validation reports, and an `audit/` bundle with the retrieval audit, ledgers, review loop, and execution provenance/transcripts. For Max, `validation-report.json.measurements.max_review_loop` reports scale and state; root-level `review_outcomes` recomputes decision path, concerns, review-triggered retrieval cycles, sources/Claims added, Claims downgraded or removed, sections revised, STORM conflicts closed, blind spots disclosed, and final integrity. These audit facts stay outside article prose and body-length accounting. Artifact-only collect remains fixture-only and requires `--allow-artifact-contract`. Collect verifies hashes and does not create a public release package.
